...
def main():
    try:
        with open('data/data.json', mode='r', encoding='utf-8') as json_file:
            data = json.load(json_file)
        html = get_instagram_html(INSTAGRAM_USERNAME)
        if(data["LAST_IMAGE_ID"] == get_last_publication_url(html)):
            print("Not new image to post in discord.")
        else:
            LAST_IMAGE_ID = get_last_publication_url(html)
...
if __name__ == "__main__":
    if os.environ.get('USERNAME') != None and os.environ.get('WEBHOOK_URL') != None:
        while True:
            main()
